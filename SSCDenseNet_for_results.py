# 构建SSCDenseNet 数据集 Indian \ PaviaU \ Salinas
...
for (FLAG,curr_train_ratio) in [(1,0.1)]:
    ...
    for curr_seed in Seed_List:
        ...
        # #保存训练集gt
        # draw_one( np.reshape(train_samples_gt,[m,n] ),'result\\'+dataset_name+'_train_gt')
        # #保存验证集gt
        # draw_one( np.reshape(val_samples_gt,[m,n] ),'result\\'+dataset_name+'_val_gt')
        # #保存测试集gt
        # draw_one( np.reshape(Test_GT,[m,n] ),'result\\'+dataset_name+'_test_gt')
        # #保存全局gt图
        # draw_one( np.reshape(train_samples_gt+test_samples_gt+val_samples_gt,[m,n] ),'result\\'+dataset_name+'_gt')
